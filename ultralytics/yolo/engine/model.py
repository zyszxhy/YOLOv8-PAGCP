...
class YOLO:
    ...
    def train(self, **kwargs):
        ...
        if overrides.get('pruning'):
            prue_model = self.model
            self.trainer.hub_session = self.session  # attach optional HUB session
            self.trainer.train(prue_model)
        elif not overrides.get('resume') and not(overrides.get('pruning')) :  # manually set model only if not resuming
            prue_model = None
