...
from ultralytics.yolo.utils.torch_utils import _make_divisible
...
class SELayer(nn.Module):
    def __init__(self, in_size, se_ratio=None, reduction=4):
        super(SELayer, self).__init__()
        if se_ratio:
            expand_size = _make_divisible(in_size * se_ratio, reduction)
        else:
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, hidden_dim, kernel_size, stride, use_se, use_hs):
        ...
        # inp = self.adjust_channels(inp, scale)
        # oup = self.adjust_channels(oup, scale)
        # hidden_dim = self.adjust_channels(hidden_dim, scale)
...
class GhostBottleneckV2(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, mid_chs, dw_kernel_size=3, se_ratio=0.,
                 stride=1,layer_id=None, args=None):
        ...
        if has_se:
            self.se = SELayer(mid_chs, se_ratio=se_ratio)
        else:
            self.se = None
